...
> = (props) => {
  ...
  const [dataInitPosts, setDataInitPosts] = useState<TPostCard[] | null>();
  const [dataInitErrors, setDataInitErrors] = useState<any[] | null>();
  const [dataInitQueryVariable, setDataInitQueryVariable] = useState<
    any | null
  >();
  const [dataInitPageInfo, setDataInitPageInfo] = useState<{
    endCursor: string;
    hasNextPage: boolean;
  } | null>();
  ...
  useEffect(() => {
    ...
    setDataInitPosts(dataInitPosts);
    setDataInitErrors(dataInitErrors);
    setDataInitQueryVariable(dataInitQueryVariable);
    setDataInitPageInfo(dataInitPageInfo);
  }, []);
